...
// Last data row currently in the sheet (header is row 1, data 2+). Used at
// startup to ignore the existing "previous sheet" backlog: the bot then only
// processes rows that appear ABOVE this line (new orders append at the bottom).
async function getMaxDataRow() {
  const res = await sheets().spreadsheets.values.get({
    spreadsheetId: spreadsheetId(),
    range: dataRange(),
  });
